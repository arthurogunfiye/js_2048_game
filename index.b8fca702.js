...
const updateStatus = ()=>{
    if (game.getStatus() === Game.statuses.LOSE) messageLose.classList.remove("hidden");
...
class Game {
    ...
    merge(line) {
        for(let i = 0; i < this.size - 1; i++)if (line[i] !== 0 && line[i] === line[i + 1]) {
            line[i] *= 2;
    ...
    compress(line) {
        return line.filter((value)=>value !== 0).concat(Array(this.size).fill(0)).slice(0, this.size);
    }
    transpose(state) {
    ...
    updateStatus() {
        for(let row = 0; row < this.size; row++)for(let cell = 0; cell < this.size; cell++){
            ...
            if (row < this.size - 1 && currentValue === this.state[row + 1][cell] || cell < this.size - 1 && currentValue === this.state[row][cell + 1]) return;
        }
    ...
    addRandomTile() {
        ...
        const { row, cell } = emptyCells[Math.floor(Math.random() * emptyCells.length)];
